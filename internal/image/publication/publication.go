...
// Identity binds a publication payload to its complete bytes.
type Identity struct {
	SHA256 string
	Size   int64
}

// Publisher atomically moves one staging entry only when its final destination
// does not exist. Both names are relative to directory.
type Publisher func(directory *os.File, source, destination string) error
...
type noReplacePublisher = Publisher

// IdentifyBytes returns the canonical identity of an in-memory publication.
func IdentifyBytes(data []byte) Identity {
	identity := identifyPublicationBytes(data)
	return Identity{SHA256: identity.digest, Size: identity.size}
}

// Publish durably stages and exclusively publishes one exact manifest,
// journal, and ISO set. The ISO is published last as the commit marker.
func Publish(
	sourceISO string,
	destinationISO string,
	manifestBytes []byte,
	journalBytes []byte,
	expectedISO Identity,
	expectedManifest Identity,
	expectedJournal Identity,
	publisher Publisher,
) (string, string, error) {
	return publishImageOutputs(
		sourceISO,
		destinationISO,
		manifestBytes,
		journalBytes,
		publicationIdentity{digest: expectedISO.SHA256, size: expectedISO.Size},
		publicationIdentity{digest: expectedManifest.SHA256, size: expectedManifest.Size},
		publicationIdentity{digest: expectedJournal.SHA256, size: expectedJournal.Size},
		publisher,
	)
}

// RequireAbsentPath rejects every existing filesystem object during the early
// command preflight that runs before a publication directory is open.
func RequireAbsentPath(path, label string) error {
	return requireAbsentPublicationPath(path, label)
}

// PublishOutputNoReplace exposes the platform-specific descriptor-relative
// no-replace rename for adapter compatibility tests.
func PublishOutputNoReplace(directory *os.File, source, destination string) error {
	return publishOutputNoReplace(directory, source, destination)
}
