...
import (
	"bytes"
	"context"
	"crypto/md5" //nolint:gosec // Debian's media-check format requires MD5.
	"errors"
	"fmt"
	"io"
	"os"
...
// buildManifest records live-boot identity, installer identity and both EFI boot
...
func buildManifest(request Request, layout sourceLayout, workspace string, source imagecontract.ArtifactRecord, companion imagecontract.CompanionBundleRecord) (imagecontract.Manifest, error) {
	manifest := imagecontract.Manifest{
		...
		BootArguments: strings.Fields(requiredBootArguments + " " + surfaceKernelArguments),
	}
	var err error
	manifest.BootArtifacts.Kernel, err = recordFile(filepath.Join(workspace, "live-vmlinuz"), layout.kernel)
	if err != nil {
		return manifest, err
	}
	manifest.BootArtifacts.Initrd, err = recordFile(filepath.Join(workspace, "live-initrd"), layout.initrd)
	if err != nil {
		return manifest, err
	}
	...
	uuid, err := imagecontract.ReadBoundedExtractedFile(workspace, "live-uuid", 64)
	if err != nil {
		return manifest, err
	}
	contract, err := newDirectHybrid(uuid)
	if err != nil {
		return manifest, err
	}
	marker, err := recordFile(filepath.Join(workspace, "live-uuid"), mediumIdentityPath)
	if err != nil {
		return manifest, err
	}
	...
	for _, member := range []struct{ role, file, mediaPath string }{
		...
		{"installed-grub-package", grubSupportDirectory + "/" + grubSupportDebName, grubSupportDirectory + "/" + grubSupportDebName},
	} {
...
func manifestBytes(manifest imagecontract.Manifest) ([]byte, error) {
	...
	if output.Len() == 0 || output.Len() > imagecontract.MaximumManifestSize {
		return nil, errors.New("generated Debian manifest exceeds its size bound")
	}
	return output.Bytes(), nil
}

// updateMediaChecksums preserves the source inventory and replaces changed
// members. MD5 is only Debian's legacy media-check format; SHA-256 binds all Lexr
...
func updateMediaChecksums(workspace string, replacements map[string]string) error {
	...
	for _, line := range strings.Split(string(data), "\n") {
		...
		if len(fields) != 2 || len(fields[0]) != 32 || !strings.HasPrefix(fields[1], "./") || !safeChecksumMember(strings.TrimPrefix(fields[1], "./")) {
			return errors.New("unsupported Debian MD5 inventory line")
		}
	...
	for member, file := range replacements {
		...
		hash := md5.New() //nolint:gosec // Required for Debian's md5sum.txt.
...
// validateMediaChecksums verifies the source's legacy MD5 inventory for every
// replaced member. SHA-256 remains the independent trust and publication digest.
func validateMediaChecksums(workspace string, replacements map[string]string) error {
	data, err := imagecontract.ReadBoundedExtractedFile(workspace, "md5sum.txt", 4<<20)
	if err != nil {
		return err
	}
	...
	for _, line := range strings.Split(string(data), "\n") {
		...
		fields := strings.Fields(line)
		if len(fields) != 2 || len(fields[0]) != 32 || strings.Trim(fields[0], "0123456789abcdef") != "" || !strings.HasPrefix(fields[1], "./") || !safeChecksumMember(strings.TrimPrefix(fields[1], "./")) {
			return errors.New("invalid Debian media checksum inventory")
		}
		member := strings.TrimPrefix(fields[1], "./")
		if _, ok := entries[member]; ok {
			return errors.New("duplicate Debian media checksum entry")
		}
		entries[member] = fields[0]
	}
	for member, relative := range replacements {
		if err := imagecontract.ValidateExtractedRegularFiles(workspace, []string{relative}); err != nil {
			return err
		}
		file, err := os.Open(filepath.Join(workspace, filepath.FromSlash(relative)))
		if err != nil {
			return err
		}
		hash := md5.New() //nolint:gosec // The original media checker uses MD5.
		_, readErr := io.Copy(hash, file)
		if err := errors.Join(readErr, file.Close()); err != nil {
			return err
		}
		if entries[member] != fmt.Sprintf("%x", hash.Sum(nil)) {
			return fmt.Errorf("media checksum differs for %s", member)
		}
	}
	return nil
}
