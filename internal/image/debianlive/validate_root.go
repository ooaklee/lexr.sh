...
`
	...
	if err := sp11.ValidateEarlyModules(ctx, v.Docker, toolsImage, workspace, volume, abi); err != nil {
		return err
	}
	for _, member := range []struct{ path, expected string }{
		...
		{"etc/initramfs-tools/hooks/lexr-sp11-modules", sp11.EarlyModuleHook()},
	} {
	...
	for _, input := range sp11.SupplementalGPUFirmwareInputs() {
		if input.Path != "qcom/gen70500_gmu.bin" && input.Path != "qcom/gen70500_sqe.fw" {
			continue
		}
		if err := sp11.ValidateFirmwareCopies(firmwareRoot, sections, abi, input.Path, input.SHA256, input.Size); err != nil {
			return err
		}
	}
