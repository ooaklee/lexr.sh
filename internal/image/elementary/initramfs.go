...
func buildInitramfs(ctx context.Context, docker *platform.Docker, image, workspace, volume, abi string) error {
	if !kernelABIPattern.MatchString(abi) {
		return fmt.Errorf("invalid elementary initramfs kernel ABI")
	}
...
# The single elementary filesystem is also the installer payload. Temporarily hide
...
# elementary's stock hook generates conf/uuid.conf only when this variable is set.
...
`
	if err := docker.RunInWorkspaceVolume(ctx, image, workspace, volume,
		"bash", "-ceu", script, "lexr-elementary-initramfs", abi); err != nil {
		return fmt.Errorf("generate separate elementary live and installed initramfs images: %w", err)
	}
	return nil
}
