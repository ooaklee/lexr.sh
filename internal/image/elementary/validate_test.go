...
// elementaryManifestFixture produces canonical provenance with a non-v23 kernel so
// validation cannot accidentally turn the initial test baseline into a gate.
func elementaryManifestFixture(t *testing.T) imagecontract.Manifest {
	t.Helper()
	...
	if err := os.WriteFile(filepath.Join(workspace, "casper-uuid-generic"), []byte("12345678-1234-1234-1234-123456789abc"+"\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	layout := sourceLayout{liveDirectory: "casper"}
...
// TestElementaryManifestRequiresAllBootContracts checks canonical round-tripping and
// rejects omitted, duplicate, cross-distro, unsafe and mismatched evidence.
func TestElementaryManifestRequiresAllBootContracts(t *testing.T) {
	valid := elementaryManifestFixture(t)
	...
	for _, testcase := range []struct {
		...
		{"wrong kernel path", func(m *imagecontract.Manifest) { m.BootArtifacts.Kernel.Path = "live/vmlinuz" }},
...
// TestElementaryInstalledInitramfsCannotRequireTheUSB detects a live-initramfs mix-up
// and wrong-ABI modules while allowing the separate installed-system image.
func TestElementaryInstalledInitramfsCannotRequireTheUSB(t *testing.T) {
