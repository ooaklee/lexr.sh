...
func parseBootLayout(report string, imageSize int64) (bootExtent, error) {
	if imageSize <= 0 || imageSize > maximumISOBytes || imageSize%512 != 0 ||
		...
		return bootExtent{}, errors.New("elementary output lacks its partition-relative ISO and GPT EFI partition")
	}
	matches := gptExtentPattern.FindAllStringSubmatch(report, -1)
	if len(matches) != 2 || matches[0][1] != "1" || matches[1][1] != "2" {
		return bootExtent{}, errors.New("elementary output has unexpected or ambiguous GPT partitions")
	}
	...
	if iso.offset != 32768 || iso.offset+iso.size != esp.offset || esp.offset%2048 != 0 || esp.size > 64<<20 {
		return bootExtent{}, errors.New("elementary output has overlapping, unaligned or oversized EFI boot extents")
	}
	boots := elToritoPattern.FindAllStringSubmatch(report, -1)
	if len(boots) != 1 || strings.Count(report, "El Torito boot img") != 1 {
		return bootExtent{}, errors.New("elementary output requires exactly one non-emulated UEFI boot image")
	}
	...
	if loadErr != nil || lbaErr != nil || loadSectors != esp.size/512 || lba != esp.offset/2048 {
		return bootExtent{}, errors.New("elementary optical and USB boot paths select different EFI images")
	}
	return esp, nil
}
...
func validateESPInventory(listing string) error {
	...
	if !reflect.DeepEqual(actual, expected) {
		return errors.New("elementary ESP contains an unexpected loader or configuration")
	}
	return nil
}
