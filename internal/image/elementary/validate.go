...
func (v *Validator) Validate(ctx context.Context, isoPath string) (report imagecontract.ValidationReport, resultErr error) {
	...
	if err := check("supplemental-gpu-firmware", validateFirmwareDirectory(workspace, "sp11/firmware"), "pinned GPU firmware and complete upstream licence evidence agree"); err != nil {
		return report, err
	}
