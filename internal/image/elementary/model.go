...
// Request carries resolved inputs and publication policy for a elementary image.
type Request struct {
	SourceISO          string
	SourceSHA256       string
	OutputISO          string
...
// Remasterer coordinates elementary-specific filesystem and bootloader preparation.
...
// creationSteps records elementary's actual single-root workflow. Journal validators
// must select this adapter's order, not infer a workflow from its step count.
var creationSteps = []plan.Step{
	{ID: "verify-source", Kind: "verify", Description: "Verify the elementary OS ARM64 source ISO"},
	...
	{ID: "extract-live-root", Kind: "extract", Description: "Validate elementary's fixed Casper directory and extract its single filesystem"},
	...
	{ID: "install-kernel", Kind: "kernel", Description: "Register the custom kernel and install elementary's GRUB support"},
	...
	{ID: "repack-live-root", Kind: "filesystem", Description: "Repack elementary's installer filesystem and update its package inventory"},
	{ID: "create-hybrid-boot", Kind: "boot", Description: "Create the GPT EFI partition and direct GRUB live boot paths"},
	{ID: "validate-output", Kind: "verify", Description: "Validate elementary's media, installer payload and boot-artifact agreement"},
