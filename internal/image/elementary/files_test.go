...
// TestMediaChecksumsPreserveUnchangedSourceMembers checks elementary's legacy media
...
func TestValidationRejectsStaleMediaChecksums(t *testing.T) {
	root := t.TempDir()
	if err := os.WriteFile(filepath.Join(root, "md5sum.txt"), []byte("900150983cd24fb0d6963f7d28e17f72  ./casper/vmlinuz\n"), 0644); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(root, "kernel"), []byte("abc"), 0644); err != nil {
		t.Fatal(err)
	}
	mappings := map[string]string{"casper/vmlinuz": "kernel"}
