...
import (
	"bytes"
	"context"
	"crypto/md5" //nolint:gosec // elementary's media-check format requires MD5.
	"errors"
	"fmt"
	"io"
	"os"
...
func manifestBytes(manifest imagecontract.Manifest) ([]byte, error) {
	...
	if output.Len() == 0 || output.Len() > imagecontract.MaximumManifestSize {
		return nil, errors.New("generated elementary manifest exceeds its size bound")
	}
	return output.Bytes(), nil
}

// updateMediaChecksums preserves the source inventory and replaces changed
// members. MD5 is only elementary's legacy media-check format; SHA-256 binds all Lexr
...
func updateMediaChecksums(workspace string, replacements map[string]string) error {
	...
	for _, line := range strings.Split(string(data), "\n") {
		...
		if len(fields) != 2 || len(fields[0]) != 32 || !strings.HasPrefix(fields[1], "./") || !safeChecksumMember(strings.TrimPrefix(fields[1], "./")) {
			return errors.New("unsupported elementary MD5 inventory line")
		}
	...
	for member, file := range replacements {
		...
		hash := md5.New() //nolint:gosec // Required for elementary's md5sum.txt.
...
func validateMediaChecksums(workspace string, replacements map[string]string) error {
	...
	for _, line := range strings.Split(string(data), "\n") {
		...
		if len(fields) != 2 || len(fields[0]) != 32 || strings.Trim(fields[0], "0123456789abcdef") != "" || !strings.HasPrefix(fields[1], "./") || !safeChecksumMember(strings.TrimPrefix(fields[1], "./")) {
			return errors.New("invalid elementary media checksum inventory")
		}
		member := strings.TrimPrefix(fields[1], "./")
		if _, ok := entries[member]; ok {
			return errors.New("duplicate elementary media checksum entry")
		}
