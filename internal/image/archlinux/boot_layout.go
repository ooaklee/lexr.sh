...
func parseBootLayout(report string, imageSize int64) (bootExtent, error) {
	if imageSize <= 0 || imageSize > maximumImageBytes || imageSize%512 != 0 ||
		...
		return bootExtent{}, errors.New("Arch output lacks its partition-relative ISO and GPT EFI partition")
	}
	matches := gptExtentPattern.FindAllStringSubmatch(report, -1)
	if len(matches) != 2 || matches[0][1] != "1" || matches[1][1] != "2" {
		return bootExtent{}, errors.New("Arch output has unexpected or ambiguous GPT partitions")
	}
	...
	if iso.offset != 32768 || iso.offset+iso.size != esp.offset || esp.offset%2048 != 0 || esp.size > 64<<20 {
		return bootExtent{}, errors.New("Arch output has overlapping, unaligned or oversized EFI boot extents")
	}
	boots := elToritoPattern.FindAllStringSubmatch(report, -1)
	if len(boots) != 1 || strings.Count(report, "El Torito boot img") != 1 {
		return bootExtent{}, errors.New("Arch output requires exactly one non-emulated UEFI boot image")
	}
	...
	if loadErr != nil || lbaErr != nil || loadSectors != esp.size/512 || lba != esp.offset/2048 {
		return bootExtent{}, errors.New("Arch optical and USB boot paths select different EFI images")
	}
	return esp, nil
}

// validateESPInventory rejects an uninspected firmware executable or an
// alternate configuration in the FAT partition used by optical and USB boot.
func validateESPInventory(listing string) error {
	expected := []string{"::/efi/", "::/boot/", "::/efi/boot/", "::/efi/boot/bootaa64.efi", "::/boot/grub/", "::/boot/grub/grub.cfg"}
	actual := strings.Fields(strings.ToLower(listing))
	sort.Strings(actual)
	sort.Strings(expected)
	if !reflect.DeepEqual(actual, expected) {
		return errors.New("Arch ESP contains an unexpected loader or configuration")
	}
	return nil
}
