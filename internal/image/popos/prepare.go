...
// inspectedGRUBSHA256 identifies Pop generic-3's direct ARM64 GRUB. Its
...
// extractSource preserves Pop's installer identity and versioned directory,
// rejecting an alternate GRUB route or a changed installer symlink contract.
func (r *Remasterer) extractSource(ctx context.Context, toolsImage, workspace, volume string) (sourceLayout, error) {
	if err := r.Docker.RunInWorkspaceAsHostUser(ctx, toolsImage, workspace,
		...
		"-extract", "/md5sum.txt", "/work/source-md5sum.txt",
		"-extract", "/casper", "/work/source-casper"); err != nil {
	...
	target, err := os.Readlink(filepath.Join(workspace, "source-casper"))
	if err != nil || target != layout.liveDirectory {
		return layout, errors.New("Pop's /casper installer symlink does not match the boot directory")
	}
	if _, err := os.Lstat(filepath.Join(workspace, "source-grub", "arm64-efi", "grub.cfg")); !errors.Is(err, os.ErrNotExist) {
		return layout, errors.New("unsupported alternate Pop GRUB configuration route")
	}
	...
	if digest != inspectedGRUBSHA256 {
		return layout, errors.New("Pop GRUB bootstrap has changed; inspect the new source before remastering")
	}
	arguments := []string{"xorriso", "-osirrox", "on", "-indev", "/work/source.iso"}
	for _, member := range []string{"filesystem.squashfs", "filesystem.manifest", "filesystem.manifest-remove", "filesystem.size"} {
	...
	if err := imagecontract.ValidateExtractedRegularFiles(workspace, []string{"source-filesystem.squashfs", "source-filesystem.manifest", "source-filesystem.manifest-remove", "source-filesystem.size"}); err != nil {
		return layout, err
	}
...
test -f "$root/etc/pop-os/os-release"
grep -qx 'ID=pop' "$root/etc/pop-os/os-release"
...
test ! -e "$root/etc/initramfs/post-update.d/zzz-lexr-pop-boot-refresh"
test ! -e "$root/usr/libexec/lexr/pop-boot-refresh"
`
	if err := r.Docker.RunInWorkspaceVolume(ctx, toolsImage, workspace, volume, "bash", "-ceu", script); err != nil {
		return layout, fmt.Errorf("unsupported Pop installer root: %w", err)
	}
	return layout, nil
}
...
// bindMedia uses the UUID actually emitted by Pop's Casper hook. The Linux
