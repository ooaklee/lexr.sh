...
// Request carries resolved inputs and publication policy for a Pop image.
type Request struct {
	SourceISO          string
	SourceSHA256       string
	OutputISO          string
...
// Remasterer coordinates Pop-specific filesystem and bootloader preparation.
...
// creationSteps records Pop's actual single-root workflow. Journal validators
// must select this adapter's order, not infer a workflow from its step count.
var creationSteps = []plan.Step{
	{ID: "verify-source", Kind: "verify", Description: "Verify the Pop!_OS ARM64 source ISO"},
	...
	{ID: "extract-live-root", Kind: "extract", Description: "Validate Pop's versioned Casper directory and extract its single filesystem"},
	{ID: "prepare-wifi", Kind: "firmware", Description: "Derive SP11 board data from Pop's firmware before the first Wi-Fi probe"},
	{ID: "install-kernel", Kind: "kernel", Description: "Register the custom kernel and install Pop's systemd-boot support"},
	...
	{ID: "bind-live-media", Kind: "boot", Description: "Bind the generated Casper UUID to the direct ISO medium"},
	{ID: "pair-device-trees", Kind: "device-tree", Description: "Stage the matching X1E and X1P device trees"},
	{ID: "repack-live-root", Kind: "filesystem", Description: "Repack Pop's installer filesystem and update its package inventory"},
	{ID: "create-hybrid-boot", Kind: "boot", Description: "Create the GPT EFI partition and direct GRUB live boot paths"},
	{ID: "validate-output", Kind: "verify", Description: "Validate Pop's media, installer payload and boot-artifact agreement"},
