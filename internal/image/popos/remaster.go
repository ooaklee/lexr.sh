...
// Create prepares Pop's single installer filesystem, creates a USB-visible
...
func (r *Remasterer) Create(ctx context.Context, request Request) (result Result, resultErr error) {
	...
	for _, suffix := range []string{"", ".manifest.json", ".journal.json"} {
		if err := imagecontract.RequireAbsentPublication(output+suffix, "Pop image output"); err != nil {
			return result, err
		}
	}
	...
	workspace, err := os.MkdirTemp(parent, ".lexr-pop-")
	if err != nil {
		return result, err
	}
	...
	progress("Verifying private copies of the Pop source and kernel packages")
	...
	defer func() {
		...
		if err := r.Docker.RemoveWorkVolume(cleanup, volume); err != nil {
			resultErr = errors.Join(resultErr, fmt.Errorf("remove Pop workspace volume %s: %w", volume, err))
		}
	}()
	if err := checkpoint("prepare-tools", nil); err != nil {
		return result, err
	}
	progress("Inspecting Pop's versioned Casper directory and installer filesystem")
	...
	progress("Installing the custom kernel and Pop installed-system support")
	...
	if err := installRecoverySupport(ctx, r.Docker, toolsImage, workspace, volume, encoded, support.Included); err != nil {
		return result, err
	}
	progress("Repacking Pop's installer filesystem, recovery support and package inventory")
	...
	progress("Independently validating the complete Pop image")
	...
	if !validation.Valid {
		return result, errors.New("Pop image validation did not pass")
	}
