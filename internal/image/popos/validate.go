...
func (v *Validator) Validate(ctx context.Context, isoPath string) (report imagecontract.ValidationReport, resultErr error) {
	...
	workspace, err := os.MkdirTemp(filepath.Dir(absolute), ".lexr-pop-validate-")
	if err != nil {
		return report, err
	}
	defer func() {
	...
	check := func(name string, err error, detail string) error {
		...
		if err != nil {
			return fmt.Errorf("Pop validation %s: %w", name, err)
		}
		return nil
	}
	...
	if err := extract("-extract", "/sp11", "/work/sp11", "-extract", "/boot/grub", "/work/grub", "-extract", "/.disk", "/work/disk", "-extract", "/casper", "/work/casper-link", "-extract", "/efi/boot/bootaa64.efi", "/work/bootaa64.efi"); err != nil {
	...
	if err := check("embedded-manifest", err, "canonical Pop kernel, boot and media-discovery contract"); err != nil {
		return report, err
	}
	...
	link, linkErr := os.Readlink(filepath.Join(workspace, "casper-link"))
	if linkErr != nil || link != layout.liveDirectory {
		return report, check("installer-casper-link", errors.New("installer /casper link does not select the boot filesystem"), "")
	}
	check("installer-casper-link", nil, "Distinst sees the same single filesystem as the live kernel")
	...
	if _, err := os.Lstat(filepath.Join(workspace, "grub/arm64-efi/grub.cfg")); !errors.Is(err, os.ErrNotExist) {
		return report, check("grub-bootstrap", errors.New("alternate GRUB route overrides the validated menu"), "")
	}
	...
	if err := check("installer-product", err, "Pop ARM64 installer identity preserved"); err != nil {
		return report, err
	}
	...
	if err := check("native-installer-recovery", v.validateRecoverySupport(ctx, toolsImage, workspace, volume, manifest), "Distinst recovery hook, original live-media DTBs and companion are retained in the installer root"); err != nil {
		return report, err
	}
	report.Valid = true
	return report, nil
...
func validateManifest(manifest imagecontract.Manifest) (sourceLayout, caspermedia.Contract, []imagecontract.ArtifactRecord, error) {
	...
	if manifest.SchemaVersion != imagecontract.ManifestSchemaVersion || manifest.Adapter != AdapterID || manifest.Layout != "hybrid-iso" || manifest.SecureBoot != secureBootPolicy || manifest.ToolVersion == "" || manifest.CreatedAt.IsZero() {
		return layout, empty, nil, errors.New("unsupported or incomplete Pop manifest identity")
	}
	if !kernelABIPattern.MatchString(manifest.KernelBundle.ABI) || manifest.KernelBundle.EffectiveDTBDelivery != kernel.DTBDeliveryExternalRequired {
		return layout, empty, nil, errors.New("Pop requires a portable exact ABI and external-required DTBs")
	}
	...
	for _, evidence := range media.Evidence {
		if seen[evidence.Role] {
			return layout, empty, nil, errors.New("duplicate Pop media-discovery role")
		}
		...
		case "live-directory":
			if evidence.Scope != "iso-filesystem" || !liveDirectoryPattern.MatchString(evidence.Path) || evidence.Value != "/"+evidence.Path || evidence.Artifact != nil {
				return layout, empty, nil, errors.New("invalid Pop live-directory evidence")
			}
		...
		case "efi-bootloader", "efi-system-partition", "installer-product":
			...
			if evidence.Scope != "iso-filesystem" || evidence.Path != expected || evidence.Value != "" || evidence.Artifact == nil || evidence.Artifact.Path != expected {
				return layout, empty, nil, errors.New("invalid Pop EFI or installer evidence")
			}
			records = append(records, *evidence.Artifact)
		default:
			return layout, empty, nil, errors.New("unrecognised Pop media-discovery role")
		}
	}
	if len(seen) != 6 || layout.liveDirectory == "" {
		return layout, empty, nil, errors.New("incomplete Pop media-discovery evidence")
	}
	...
	if manifest.BootArtifacts.Kernel.Path != layout.kernel || manifest.BootArtifacts.Initrd.Path != layout.initrd {
		return layout, empty, nil, errors.New("Pop boot artefacts do not use the declared live directory")
	}
	...
	if !reflect.DeepEqual(manifest.BootArguments, expectedArgs) {
		return layout, empty, nil, errors.New("Pop kernel arguments differ from the adapter contract")
	}
	if len(manifest.BootArtifacts.DTBs) != len(manifest.KernelBundle.DeviceTrees) {
		return layout, empty, nil, errors.New("Pop boot DTB inventory differs from the kernel bundle")
	}
	...
	for index, tree := range manifest.KernelBundle.DeviceTrees {
		...
		if actual.Path != "sp11/dtb/"+tree.Basename || actual.SHA256 != tree.SHA256 {
			return layout, empty, nil, errors.New("Pop boot DTB identity differs from the kernel bundle")
		}
	...
	for _, present := range required {
		if !present {
			return layout, empty, nil, errors.New("Pop SP11 DTB inventory is incomplete")
		}
	}
