...
func validate(raw document, entries []Entry) error {
	...
	for index, rawEntry := range raw.Entries {
		...
		if validateByteLength(add, prefix+".adapter", string(entry.Adapter), maximumIdentifierBytes) {
			...
			case AdapterNone, AdapterUbuntuCasper, AdapterFedoraLive, AdapterElementaryCasper, AdapterDebianLive:
				adapterValid = true
			default:
				add(prefix+".adapter", "must be %q, %q, %q, %q, or %q, got %q", AdapterNone, AdapterUbuntuCasper, AdapterFedoraLive, AdapterElementaryCasper, AdapterDebianLive, entry.Adapter)
			}
		}
