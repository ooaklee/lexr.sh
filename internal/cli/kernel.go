...
func (a *application) newKernelBuildCommand() *cobra.Command {
	...
	bootImageMode := string(kernelbuild.BootImageModeSource)
	command := &cobra.Command{
		Use:   "build",
		...
		RunE: func(command *cobra.Command, _ []string) error {
			if strings.TrimSpace(request.SourceDirectory) != "" &&
				!command.Flags().Changed("git-url") && !command.Flags().Changed("git-branch") {
				request.GitURL = ""
				request.GitBranch = ""
			}
			request.BootImageMode = kernelbuild.BootImageMode(bootImageMode)
	...
	command.Flags().StringVar(&request.GitURL, "git-url", kernelbuild.DefaultGitURL, "HTTPS kernel source repository")
	command.Flags().StringVar(&request.GitBranch, "git-branch", kernelbuild.DefaultGitBranch, "kernel source branch or tag")
	command.Flags().StringVar(&request.SourceDirectory, "source-dir", "", "clean local Git worktree whose exact HEAD commit should be built")
	command.Flags().StringVar(&bootImageMode, "boot-image-mode", string(kernelbuild.BootImageModeSource), "boot image policy: source, stubble, or nostubble")
...
func (a *application) writeKernelBuildReceipt(receipt kernelbuild.Receipt) error {
	if receipt.Plan.DryRun {
		if receipt.Plan.SourceKind == kernelbuild.SourceKindLocalGitCommit {
			_, err := fmt.Fprintf(a.out,
				"kernel build dry run\nrepository root: %s\nsource kind: %s\nsource directory: %s\nplanned local source revision: %s\nplanned source tree: %s\nsource files: %d\nboot image mode: %s\nwork directory: %s\noutput directory: %s\ncontainer: %s\nbuild target: %s\nminimum free space: %d GiB\nwork volume: %s\nrecipe SHA-256: %s\njobs: %d\nreset source: %t\nskip clean: %t\nconditional external-DTB packaging commands: %d\nno changes were made\n",
				receipt.Plan.RepositoryRoot, receipt.Plan.SourceKind, receipt.Plan.SourceDirectory,
				receipt.Plan.LocalSourceRevision, receipt.Plan.SourceTree, receipt.Plan.SourceFileCount,
				receipt.Plan.BootImageMode, receipt.Plan.WorkDirectory, receipt.Plan.OutputDirectory,
				receipt.Plan.ContainerImage, receipt.Plan.BuildTarget, receipt.Plan.MinimumFreeGiB,
		...
		_, err := fmt.Fprintf(a.out,
			"kernel build dry run\nrepository root: %s\nsource kind: %s\nsource: %s\nref: %s\nboot image mode: %s\nwork directory: %s\noutput directory: %s\ncontainer: %s\nbuild target: %s\nminimum free space: %d GiB\nwork volume: %s\nrecipe SHA-256: %s\njobs: %d\nreset source: %t\nskip clean: %t\nconditional external-DTB packaging commands: %d\nno changes were made\n",
			receipt.Plan.RepositoryRoot, receipt.Plan.SourceKind, receipt.Plan.GitURL, receipt.Plan.GitRef,
	...
	_, err := fmt.Fprintf(a.out,
		...
		receipt.Provenance.SourceKind, receipt.Provenance.Revision, receipt.Provenance.Tree, receipt.Provenance.BootImageMode,
