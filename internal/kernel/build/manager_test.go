...
func TestCompiledRecipeContainsOnlyNativeBuildPolicy(t *testing.T) {
	...
	for _, required := range []string{
		...
		`iflag=skip_bytes,count_bytes`, `tarfile.open(fileobj=process.stdout, mode="r|")`,
		`MAX_DTB_MEMBERS = 4096`, `MAX_DTB_BYTES = 512 * 1024 * 1024`,
		`MAX_DTB_FILE_BYTES = 4 * 1024 * 1024`, `MAX_DTB_PATH_BYTES = 1024`,
		`MAX_EMBEDDED_SECTIONS = 64`, `MAX_EMBEDDED_BYTES = MAX_EMBEDDED_SECTIONS * MAX_DTB_FILE_BYTES`,
		`embedded DTB section {number} matches more than one packaged path`, `Required DTB $stable_id`,
