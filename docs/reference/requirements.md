...
| Create or validate an implemented, experimental Ubuntu or elementary OS image; investigate Fedora with a [custom catalogue](../user-guide/installation-media.md#fedora-workstation-live-44) | Linux or macOS; Docker CLI, running daemon and Linux ARM64 container support | At least 24 GiB free workspace; the readiness check fails below it. Network is needed for remote image or kernel acquisition and on first use unless the pinned tooling image is already cached. Physical bootability remains a separate qualification gate | Regular user when inputs are readable |
