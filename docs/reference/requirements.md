...
| Create or validate an implemented Ubuntu or Fedora image | Linux or macOS; Docker CLI, running daemon and Linux ARM64 container support | At least 24 GiB free workspace; the readiness check fails below it. Network is needed for remote image or kernel acquisition and on first use unless the pinned tooling image is already cached | Regular user when inputs are readable |
