...
| Create or validate an implemented Ubuntu or Fedora image | Linux or macOS; Docker CLI, running daemon and Linux ARM64 container support | At least 24 GiB free workspace; the readiness check fails below it. Network is needed only for remote image or kernel acquisition | Regular user when inputs are readable |
